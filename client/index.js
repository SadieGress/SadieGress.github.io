import { loadTerminal, cowSay, version, changelog, helptext, setColor, cd } from "./terminalCommands.js";
...
function useTerminal(text, textBox) {
    ...
    const searchMap = {
        ...
        "cd": () => pushToTerminal(cd(query[1]), textBox)
    }
    
    if (searchMap[query[0].toLowerCase()]) {
        searchMap[query[0].toLowerCase()]();
    } else {
        pushToTerminal("Invalid command. Type 'help' for a list of valid commands.", textBox);
    }
